...
import com.fjcw.cms.util.RegexUtils;
...
public class ManageFolderAction extends ManageBaseAction {
	...
	public JsonVo<String> add(
			...
			@RequestParam(value = "folderTitle") String folderTitle,
...
//			} 
			...
			folderService.addFolder(fatherId,
					...
					SSUtils.toText(folderEname.toLowerCase().trim()), SSUtils.toText(folderTitle.trim()),status,
					check);
			json.setResult(true);
		} catch (Exception e) {
	...
	public JsonVo<String> update(
			...
			@RequestParam(value = "status") FolderConstant.status status,
			@RequestParam(value = "content", required = false) String content,@RequestParam(value = "file", required = false) MultipartFile file) {
		JsonVo<String> json = new JsonVo<String>();
...
//				}
			...
			folderService.updateFolderById(folderId,
					...
					SSUtils.toText(ename.trim()), status,
					content, height, width,title,file);
			json.setResult(true);
		} catch (Exception e) {
			json.setResult(false);
			json.setMsg(e.getMessage());
