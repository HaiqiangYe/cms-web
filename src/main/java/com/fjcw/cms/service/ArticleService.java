...
import com.fjcw.cms.dao.ArticleDao;
...
public class ArticleService {
	...
	public Article addArticle(long folderId, long adminId, String title,
			...
			MultipartFile file, String createTime,int recType,String role)
	...
	public Article updateArticle(long articleId, long folderId,
			...
			String time,int recType,String role) throws UploadException, IOException, FolderNotFoundException {
		Date now = new Date();
	 ...
	 * @throws ArticleNotFoundException
	 */
	public ArticleVo getArticleNextByCurrId(long articleId,long folderId)
			throws ArticleNotFoundException {
		ArticleVo articleVo = articleDao.getArticleNextByCurrId(articleId,folderId);
			return articleVo;
	}
	/**
	 * 上一篇
	 ...
	 * @throws ArticleNotFoundException
	 */
	public ArticleVo getArticlePrevByCurrId(long articleId,long folderId)
			throws ArticleNotFoundException {
		ArticleVo articleVo = articleDao.getArticlePrevByCurrId(articleId,folderId);
			return articleVo;
	}
	...
	@Cacheable(value = "article")
	public List<ArticleVo> getArticleListByPath( String path){
		return articleDao.getArticleListByPath(path);
	}
	@Cacheable(value = "article")
	public PageVo<ArticleVo> searchArticleListByKey(String path,String key,int offset,int rows) throws FolderNotFoundException{

		PageVo<ArticleVo> pageVo = new PageVo<ArticleVo>(rows);
		pageVo.setRows(rows);
		pageVo.getArgs().put("key", key);
		List<ArticleVo> articlelist = articleDao.searchArticleListByKey(path, key, offset, rows);
		pageVo.setCount(articleDao.searchArticleListByKeyCount(key));
		for (ArticleVo artcle : articlelist) {
			FolderVo artcleFolder = folderService
					.getFolderById(artcle.getFolderId());
			artcle.setFolder(artcleFolder);
	 ...
	 */
	//@Cacheable(value = "article")
	public List<ArticleVo> getArticlePageByRecType(int recType,
			int pageNum, int rows) throws FolderNotFoundException {
		List<ArticleVo> articlelist = articleDao
				.getArticleListOfDisplayByRecType(recType,
						pageNum,rows);
		for (ArticleVo artcle : articlelist) {
			FolderVo artcleFolder = folderService
					.getFolderById(artcle.getFolderId());
	...
	public void updateCheck(long articleId,
			com.fjcw.cms.constant.ArticleConstant.check check) {
